...
| **重新校正位置 re-measures the game window**, which is the player's own reason for the button: they move the game window while playing and the readout stays where the client used to be. With the game window moved by (120, 80), the readout did not follow - and the button moved it by exactly (120, 80). Moving the window back and pressing again returned the readout to its original rectangle | Recorded live, game window moved and restored | `docs/evidence/2026-09-23-config-gui-audit-live.txt`: `after moving the game window by (120, 80)`, `-> the readout followed the game window exactly ((120, 80))`, `after 重新校正位置 with the window back: rect (1348, 358, …)` |
| Before that fix the button recomputed the corner from the client rectangle **captured when the readout started**, so after a move it put the readout back on the *old* position and changed nothing | Implementation fact (the player reported the purpose, the code confirmed it) | `OverlayPresenter.reposition(window=…)`, `Watch._move(remeasure=True)` |

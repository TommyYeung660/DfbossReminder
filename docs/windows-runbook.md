...
* 位置不會自己跟著遊戲視窗跑（這個功能已經移除）。設定視窗最上面的「顯示位置」是**設定值**，
  「位置微調」的箭頭是**臨時微調**（不會存檔、overlay 重開就回到設定值）；
  **移動或縮放遊戲視窗之後，按一下「重新校正位置」**：它會重新量測遊戲視窗現在的位置，
  把 overlay 放回「對齊位置／對齊位移」設定的相對位置，並把臨時微調歸零。
  （實測：遊戲視窗移動 120,80，overlay 會跟著移動 120,80。）
...
表、開始/停止、四個位置箭頭都在，並且按一次那些不需要遊戲的按鈕。加 `--live` 會用真正的 controller 走完整個流程：按「開始」、數 overlay 視窗數目、
按四次箭頭、按「重新校正位置」（位置必須回到原本的矩形）、按「停止」（視窗數必須變 0）、
再按一次「開始」（必須還是 1 個，不是 2 個）、最後再停一次。
這幾行就是 2026-09-23 那兩個 bug 的回歸測試 —— 停止沒用、疊出第二層、視窗卡住。
